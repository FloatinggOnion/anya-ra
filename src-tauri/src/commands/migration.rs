use crate::workspace_paths::{get_anya_path, get_chat_dir, get_notes_dir, get_papers_dir};
use std::fs;
use std::path::Path;
use tauri::command;
...
pub fn check_workspace_needs_migration(workspace_path: String) -> Result<bool, String> {
    ...
    let needs_migration =
        (has_old_papers || has_old_notes || has_old_chats || has_old_graph) && has_no_anya;
...
pub fn migrate_workspace_to_anya(workspace_path: String) -> Result<(), String> {
    ...
    fs::create_dir_all(&anya_path).map_err(|e| format!("Failed to create .anya folder: {}", e))?;
